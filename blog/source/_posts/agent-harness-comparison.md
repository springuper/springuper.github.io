...
| token（约） | 16.3 万（含缓存读） | **1,762** | 1.3 万 | 3,375 | 未统计 |
...
| token（约） | 11.7 万（含缓存读） | 6,030 | 1.5 万 | **5,394** | 未统计 |
| 成本（约） | $0.197 | 极低 | 低 | **~$0.0001** | 极低 |
| 改动文件 | checkout+pricing | 全部 3 个 | 全部 3 个 | 全部 3 个 | checkout+pricing |

*（任务二的 codex 与 dsh 是在无外层沙箱的正常环境下跑的——任务一里它们的 bash 沙箱启动失败是环境限制，不是能力问题，见 3.5 节。）*
