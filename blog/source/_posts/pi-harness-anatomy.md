...
一年时间，三个包长到十一个。这条时间线基本是被需求推着走的，很少看到先画一套架构再落地。也有例外：上下文压缩在动手前两天先写了研究文档，pi-tui 至今留着一份 `tui-plan.md`，开头写着它记录的是设计讨论里的决定。但主流是前者：模型要统一，就抽 pi-ai。要有产品，就长出 coding-agent。会话要能被别人接进来，就有了 server 和 protocol。
...
最后是太长的时候。上下文快装不下时，一种常见做法是把最旧的几条消息丢掉。Pi 用摘要替换，而且原文一条不删地留在文件里：
...
但 0.00007 美元不是缓存一家的功劳。那笔账里至少还有三件事：总额只有 3375 个 token、用的是最便宜的 Flash 档、输出只有 120 个 token。缓存是其中最大的一项，不是全部。上一篇也交代过，成本是各工具自己上报的数值，只宜做量级参考。
...
- 工具本身没有确认弹窗，该跑就跑。注意这里说的是工具层：第一次在一个带 `.pi` 资源的项目里启动时，还是会有一次项目信任询问。
...
作者是游戏圈出身（libGDX 的作者），这套做法和游戏渲染管线是同一路数：每帧重算一遍，只把变了的行发出去，再用同步输出把整帧攒齐。仓库里还有个用它跑 DOOM 的示例，在 overlay 里以 35 FPS 实时渲染，算是这套思路的一个旁证。
...
这些能力后来大多由生态补上，而且多数样板就躺在官方仓库的 `examples/extensions/` 里：子代理、计划模式、待办清单，官方都给了示例，只是不装进核心。真需要成品，社区还有更完整的实现，比如把 MCP server 映射成 Pi 原生工具的那个适配器。官方负责把积木和图鉴做精，剩下的我们自己拼。
