...
2. **一批工具里只有一个说“停”，算不算停？** 不算。要这一批**全部**返回 `terminate: true`，循环才跳过紧接着的那次模型调用。这个标记最典型的用法是收尾工具：官方示例里的 `structured_output`，模型最后调一次它把结果交出来，它标上 `terminate: true`，循环就此收工，**省掉一次完整的模型往返**。反过来，同一批里如果还混着一个普通工具（比如刚跑完 `npm test`），它的输出模型还没看过，这一批就不能停，得再问一轮。
...
- 工具本身没有确认弹窗，该跑就跑。注意这里说的是工具层：第一次在一个带 `.pi` 资源（这个项目自己的扩展、提示词、技能）的目录里启动时，还是会有一次项目信任询问。
