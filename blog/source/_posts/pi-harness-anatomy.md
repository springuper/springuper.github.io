...
id 是 8 位 hex，比如 `a5c1c471`，只用来定位，不参与语义。这份文件是我用假模型在本地写的，所以 `cost` 全是 0，`usage` 里那几个数字也是示意值，真实会话里这里会填上真金白银和真实计量。而 assistant 那条消息上挂着 `usage`，里面 `cacheRead: 3000` 是一个独立字段。这一趟花了多少缓存，会话文件里就记着多少。
